...
import "../../styles/admin/AuditoriaAdminMobile.css"

function AuditoriaAdminMobile({ showToast }) {
  ...
  function obterClasseAcao(acao, status) {
    ...
    if (acaoNormalizada.includes("login")) return "login"
    if (acaoNormalizada.includes("chatbot")) return "chatbot"
    if (acaoNormalizada.includes("reserva")) return "reservas"
    if (acaoNormalizada.includes("campi") || acaoNormalizada.includes("campus")) {
      return "campi"
    }
    if (acaoNormalizada.includes("usuario")) return "usuarios"
    if (acaoNormalizada.includes("sala")) return "salas"
  ...
  const modalDetalhes =
    ...
    createPortal(
      ...
      >
        <div className="monitor-modal" onMouseDown={(e) => e.stopPropagation()}>
          ...
          <div className="monitor-modal-body">
            <div className="monitor-detail-grid">
              <p><b>Usuário:</b> {logSelecionado.nomeUsuario || "Sistema"}</p>
              <p><b>E-mail:</b> {logSelecionado.emailUsuario || "—"}</p>
              <p><b>IP:</b> {logSelecionado.ipMaquina || "—"}</p>
              <p><b>Sessão:</b> {logSelecionado.sessionId || "—"}</p>
              <p><b>Módulo:</b> {logSelecionado.modulo}</p>
              <p><b>Etapa:</b> {logSelecionado.etapa || "—"}</p>
              <p>
                <b>Status:</b>{" "}
                <span className={`monitor-status monitor-status-${obterClasseStatus(logSelecionado.status)}`}>
                  {formatarRotulo(logSelecionado.status)}
                </span>
              </p>
              <p><b>ID usuário:</b> {logSelecionado.idUsuario || "—"}</p>
            </div>
  ...
  return (
    <div className="auditoria-admin monitor-auditoria auditoria-mobile-pro">
      <section className="monitor-hero">
        <div>
          <span>Centro de monitoramento</span>
          <h2>Auditoria do Sistema</h2>
          <p>
            Acompanhe ações, erros, abandonos, IPs, sessões e eventos críticos do SIGSAS.
          </p>
        </div>
        ...
        <div className="monitor-panel status-monitor">
          ...
          <div className="monitor-donut-area">
            <div
              ...
              style={{
                background: `conic-gradient(
                  ...
                  porcentagem(resumo.sucesso || 0) + porcentagem(resumo.iniciado || 0)
                }%,
                  #ef4444 ${
                    porcentagem(resumo.sucesso || 0) + porcentagem(resumo.iniciado || 0)
                  }% ${
            ...
            <div className="monitor-legend">
              <p><i className="legend-success" /> Sucesso <b>{resumo.sucesso || 0}</b></p>
              <p><i className="legend-warning" /> Iniciado <b>{resumo.iniciado || 0}</b></p>
              <p><i className="legend-danger" /> Erro <b>{resumo.erro || 0}</b></p>
              <p><i className="legend-accent" /> Abandono <b>{resumo.abandono || 0}</b></p>
            </div>
          </div>
        </div>
            ...
            {logsPorDia.map((item) => (
              <div key={item.dia} className="timeline-item">
                <div
                  ...
                  style={{ height: `${Math.max((item.total / maiorDia) * 100, 12)}%` }}
                />
                <span>{item.dia}</span>
              </div>
            ))}
          ...
          <div className="monitor-alert-list">
            {resumo.errosRecentes.slice(0, 4).map((log) => (
              ...
              >
                ...
                <span>{log.nomeUsuario || "Sistema"} • {formatarData(log.dataHora)}</span>
              </button>
            ))}
      ...
      <section className="monitor-table-card auditoria-mobile-list-card">
        ...
        <div className="monitor-table-header auditoria-mobile-header">
          <div>
        ...
        <div className="auditoria-mobile-list">
          {logsFiltrados.map((log) => (
            <article key={log.id} className="auditoria-mobile-item">
              <div className="auditoria-mobile-item-top">
                <div>
                  <span className="auditoria-mobile-date">{formatarData(log.dataHora)}</span>
                  <h4>{log.nomeUsuario || "Sistema"}</h4>
                  <small>{log.emailUsuario || "Sem e-mail"}</small>
                </div>

                <span className={`monitor-status monitor-status-${obterClasseStatus(log.status)}`}>
                  {formatarRotulo(log.status)}
                </span>
              </div>

              <div className="auditoria-mobile-badges">
                <span
                  className={`monitor-action monitor-action-${obterClasseAcao(log.acao, log.status)}`}
                  title={log.acao || "Ação não informada"}
                >
                  {formatarRotulo(log.acao)}
                </span>

                <span className="auditoria-mobile-module">{log.modulo || "Sem módulo"}</span>
              </div>

              <div className="auditoria-mobile-grid">
                <div className="auditoria-mobile-field">
                  <span>Etapa</span>
                  <strong>{log.etapa || "—"}</strong>
                </div>

                <div className="auditoria-mobile-field">
                  <span>IP</span>
                  <strong>{log.ipMaquina || "—"}</strong>
                </div>

                <div className="auditoria-mobile-field auditoria-mobile-field-full">
                  <span>Sessão</span>
                  <strong className="auditoria-mobile-break">{log.sessionId || "—"}</strong>
                </div>
              </div>

              <button
                className="btn secondary monitor-detail-btn auditoria-mobile-btn"
                onClick={() => setLogSelecionado(log)}
              >
                Ver detalhes
              </button>
          ...
          {logsFiltrados.length === 0 && (
            <div className="monitor-empty">Nenhum log encontrado.</div>
          )}
        </div>
      </section>
...
export default AuditoriaAdminMobile
