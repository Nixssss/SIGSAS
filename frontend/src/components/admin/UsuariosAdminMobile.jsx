...
import "../../styles/admin/UsuariosAdminMobile.css"
...
function UsuariosAdminMobile({ showToast }) {
    ...
    <div className="usuarios-admin">
      <section className="usuarios-hero">
        <div className="usuarios-hero-content">
          <span className="usuarios-hero-eyebrow">Administração</span>
          <h2>Gestão de Usuários</h2>
      ...
      <section className="usuarios-list-card">
        ...
        <div className="usuarios-mobile-list" aria-live="polite">
          {usuariosFiltrados.map((usuario) => {
            const perfilClasse = obterClassePerfil(usuario.perfil)
            const nomeInstituicao =
              usuario.instituicao || getNomeInstituicao(usuario.idInstituicao)
            const cursosUsuario = montarTextoCursos(usuario)
            const totalCursos = getTotalCursosUsuario(usuario)

            return (
              <article
                key={usuario.id}
                className={`usuarios-mobile-card perfil-${perfilClasse}`}
              >
                <header className="usuarios-mobile-card-header">
                  <div className="usuario-identidade">
                    <span
                      className={`usuario-avatar perfil-${perfilClasse}`}
                      aria-hidden="true"
                    >
                  ...
                  <span className={`usuario-badge perfil-${perfilClasse}`}>
                    {formatarPerfil(usuario.perfil)}
                  </span>
                </header>

                <dl className="usuarios-mobile-details">
                  <div>
                    <dt>E-mail</dt>
                    <dd title={usuario.email}>{usuario.email || "Não informado"}</dd>
                  </div>

                  <div>
                    <dt>Matrícula</dt>
                    <dd>{usuario.matricula || "Não informada"}</dd>
                  </div>

                  <div>
                    <dt>Cargo</dt>
                    <dd>{usuario.cargo || "Não informado"}</dd>
                  </div>

                  <div>
                    <dt>Instituição</dt>
                    <dd title={nomeInstituicao}>{nomeInstituicao}</dd>
                  </div>
                </dl>

                <div className="usuarios-mobile-cursos">
                  <span>Cursos</span>
                  <p title={cursosUsuario}>{cursosUsuario}</p>
                  {totalCursos > 0 && <small>{totalCursos} curso(s) vinculado(s)</small>}
                </div>

                <footer className="usuarios-row-actions">
                  <button
                    type="button"
...
export default UsuariosAdminMobile