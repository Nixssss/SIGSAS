...
class AssuntoService:
    def get_all(self, db: Session) -> List[Assunto]:
        return db.query(Assunto).all()

    def list_all(self, db: Session) -> List[Assunto]:
        return db.query(Assunto).all()

    def get_by_id(self, db: Session, id: int) -> Optional[Assunto]:
        return db.query(Assunto).filter(Assunto.id == id).first()

    def create(self, db: Session, obj_in: AssuntoCreate) -> Assunto:
    ...
    def update(self, db: Session, id: int, obj_in: AssuntoUpdate) -> Optional[Assunto]:
