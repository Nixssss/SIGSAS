...
class FraseService:
    def list_all(self, db: Session) -> List[Frase]:
        return db.query(Frase).all()

    def get_by_id(self, db: Session, id: int) -> Optional[Frase]:
        return db.query(Frase).filter(Frase.id == id).first()

    def create(self, db: Session, obj_in: FraseCreate) -> Frase:
    ...
    def update(self, db: Session, id: int, obj_in: FraseUpdate) -> Optional[Frase]:
