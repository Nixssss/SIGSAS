...
class RespostaService:
    def get_all(self, db: Session) -> List[Resposta]:
        return db.query(Resposta).all()

    def list_all(self, db: Session) -> List[Resposta]:
        return db.query(Resposta).all()

    def get_by_id(self, db: Session, id: int) -> Optional[Resposta]:
        return db.query(Resposta).filter(Resposta.id == id).first()

    def create(self, db: Session, obj_in: RespostaCreate) -> Resposta:
    ...
    def update(self, db: Session, id: int, obj_in: RespostaUpdate) -> Optional[Resposta]:
