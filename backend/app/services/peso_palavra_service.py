...
class PesoPalavraService:
    def get_all(self, db: Session) -> List[PesoPalavra]:
        return db.query(PesoPalavra).all()

    def list_all(self, db: Session) -> List[PesoPalavra]:
        return db.query(PesoPalavra).all()

    def get_by_id(self, db: Session, id: int) -> Optional[PesoPalavra]:
        return db.query(PesoPalavra).filter(PesoPalavra.id == id).first()

    def create(self, db: Session, obj_in: PesoPalavraCreate) -> PesoPalavra:
    ...
    def update(self, db: Session, id: int, obj_in: PesoPalavraUpdate) -> Optional[PesoPalavra]:
