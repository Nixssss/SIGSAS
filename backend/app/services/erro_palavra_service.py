...
class ErroPalavraService:
    def get_all(self, db: Session) -> List[ErroPalavra]:
        return db.query(ErroPalavra).all()

    def list_all(self, db: Session) -> List[ErroPalavra]:
        return db.query(ErroPalavra).all()

    def get_by_id(self, db: Session, id: int) -> Optional[ErroPalavra]:
        return db.query(ErroPalavra).filter(ErroPalavra.id == id).first()

    def create(self, db: Session, obj_in: ErroPalavraCreate) -> ErroPalavra:
    ...
    def update(self, db: Session, id: int, obj_in: ErroPalavraUpdate) -> Optional[ErroPalavra]:
