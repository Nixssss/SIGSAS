...
class PalavraService:
    def get_all(self, db: Session) -> List[Palavra]:
        return db.query(Palavra).all()

    def list_all(self, db: Session) -> List[Palavra]:
        return db.query(Palavra).all()

    def get_by_id(self, db: Session, id: int) -> Optional[Palavra]:
        return db.query(Palavra).filter(Palavra.id == id).first()

    def create(self, db: Session, obj_in: PalavraCreate) -> Palavra:
    ...
    def update(self, db: Session, id: int, obj_in: PalavraUpdate) -> Optional[Palavra]:
